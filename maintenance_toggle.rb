...
Telegram::Bot::Client.run($token) do |bot|
  LycantululBot::Game.running.each do |game|
...
while res == 0 && (count = LycantululBot::Game.running.count) > 0
  puts "Still #{count} game(s) running, sleeping for 30 seconds"
  sleep(30)
end

Telegram::Bot::Client.run($token) do |bot|
  bot.api.send_message(chat_id: LycantululBot::RegisteredPlayer.find_by(username: 'araishikeiwai').user_id, text: 'MAINTENANCE TOGGLED') rescue nil
end
